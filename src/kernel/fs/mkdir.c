...
int sys_mkdir(const char *dirname, int mode)
{
	...
	buf = nvme_read(nvme_sector_for_block_bitmap, 1);	

	bitmap_val = *((uint16_t *) buf);

	printk("block bitmap_val = {d}  ", bitmap_val);

	// // Find first unset bit
    	// bit_pos = find_first_unset_bit(bitmap_val);

	// // Set the bit
    	// bitmap_val |= (1 << bit_pos);

	// // Write back to disk
    	// write_block_bitmap(buf, bitmap_val);
    	// // nvme_write(nvme_sector_for_block_bitmap, 1); // Write updated sector

	// printk("bit_pos = {d}  ", bit_pos);
	// printk("new bitmap val = {d}  ", bitmap_val);

	// int new_block_nr = bit_pos + 1;	/* block nr. for new dir */
...
// Write back to disk (little-endian)
void write_inode_bitmap(char *bitmap_ptr, uint16_t value) {
