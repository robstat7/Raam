...
{
	char *nvme_buffer = nvme_read(starting_sector, 5);	
	...
	block_bitmap_byte_0_val =  block_bitmap_byte_0_val | ((uint8_t) pow(2, super->first_data_block));
	...
	*block_bitmap = block_bitmap_byte_0_val;
	...
	inode[inode_nr].data_block_num = super->first_data_block;
